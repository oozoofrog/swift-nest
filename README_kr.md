...
.agents/
  skills/
    swiftnest-.../SKILL.md   # --skill-agent codex 선택 시 선택된 Docs/AI_SKILLS 기준으로 생성
    swift-master/...         # oozoofrog/codex-skills 에서 번들된 Codex 스킬
Docs/
  AI_RULES.md
  AI_WORKFLOWS.md
  AI_PROMPT_ENTRY.md
...
`Docs/`, `.swiftnest/`, 그리고 생성된 `.agents/skills/` 번들은 하네스의 핵심 자산이므로 기본적으로 버전 관리에 포함하는 것을 권장합니다.
...
4. 대상 저장소 안의 `config/project.yaml`, `AGENTS.md`, `Docs/`, 그리고 선택적으로 생성되는 `.agents/skills/` 결과를 검토합니다.
5. 대상 저장소 루트에서 에이전트 작업을 시작합니다.
6. 대상 저장소에서 생성된 `Docs/`, `.swiftnest/`, 그리고 있으면 `.agents/skills/` 를 커밋합니다.
...
6. 생성된 `Docs/`, `.swiftnest/`, 그리고 있으면 `.agents/skills/` 를 커밋합니다.
...
`--skill-agent codex` 를 선택하면 SwiftNest는 [oozoofrog/codex-skills](https://github.com/oozoofrog/codex-skills) 에서 가져온 아래 공용 Codex 스킬도 함께 설치합니다.
...
6. 생성된 `AGENTS.md`, `Docs/`, `.swiftnest/`, 그리고 선택적으로 생성되는 `.agents/skills/` 결과를 검토합니다.
7. 필요하면 명시적인 profile, skills, workflows 옵션과 함께 `swiftnest onboard` 또는 `swiftnest init` 을 다시 실행합니다.
8. `Docs/`, `.swiftnest/`, 그리고 생성된 `.agents/skills/` 는 저장소에 계속 커밋 상태로 유지합니다.
...
`--skill-agent codex` 를 선택하면 SwiftNest는 추가로 `.agents/skills/` 아래에 repo-local Codex bundle을 설치합니다.
여기에는 선택한 `Docs/AI_SKILLS/*` 에서 생성한 `swiftnest-*` 번들과 함께, [oozoofrog/codex-skills](https://github.com/oozoofrog/codex-skills) 에서 가져온 `swift-master`, `ios-swift-orchestrator`, `claude-code-bridge` 같은 공용 Codex 스킬도 포함됩니다.
